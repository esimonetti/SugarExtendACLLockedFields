...
echo 'Checking modules with SugarACLLockedFields enabled...' . PHP_EOL;
...
foreach ($full_module_list as $module => $value) {
    ...
    if (!empty($bean->object_name) && !empty($dictionary[$bean->object_name]) &&
        !empty($dictionary[$bean->object_name]['acls']) && !empty($dictionary[$bean->object_name]['acls']['SugarACLLockedFields'])) {
